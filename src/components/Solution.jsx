    ...
    <section className="bg-transparent text-slate-800 py-20">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map((b, i) => (
            <div key={i} className="flex items-start gap-3 p-4 rounded-xl border border-slate-200 bg-white/70 backdrop-blur shadow-sm">
              <span className="mt-0.5" style={{ color: GOLD }}>✔</span>
              <span className="text-slate-800">{b}</span>
            </div>
          ))}
        </div>
        <p className="text-slate-700 mt-8">Con una presencia profesional, podrás cobrar más y cerrar trabajos más grandes.</p>
      </div>
    </section>
  )
}
